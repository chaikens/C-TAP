...
NAME=NrSaDcMixedSetptsVfr

FF=ff

cp /dev/null  $NAME.fflog

if /usr/bin/time -o $NAME.time -v ${FF}mpeg  -i DS.mp4 -an -filter:v 'drawtext=fontfile=arial.ttf:text=%{n}:x=(w-tw)-140:y=(2*lh)-80:fontcolor=white:box=1:boxcolor=0x00000099:fontsize=60,scale=trunc(iw/4)*2:trunc(ih/4)*2,decimate=cycle=5,setpts=N/100/TB' -vsync vfr -y $NAME.mp4 &>> $NAME.fflog
then
