...
int main(int argc, char * argv[] )
{
  ...
  pgm = argv[0];
  if( argc != 2 ) {
    error(1,0,"Call as %s wwwwxhhhh ( < yuv-input > yuv-output ) [opts]",
	  pgm);
  }
...
static int get_our_options( int *argc, char **argv[])
{
  ...
  while (1) {
    ...
    static struct option long_options[] = {      
      ...
      {"phase-1b-file", optional_argument, 0, 0},          //1
      {"phase-1a-file", optional_argument, 0, 0},          //2
      {"circle-radius", optional_argument, 0, 0},          //3
      {"circle-border-thickness", optional_argument, 0, 0}, //4
      {"offset", optional_argument, 0, 0},                 //5
      {0,         0,                 0,  0 }
    };
