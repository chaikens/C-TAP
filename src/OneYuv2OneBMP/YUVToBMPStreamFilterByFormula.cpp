...
int main(int argc, char *argv[])
{
  ...
  if( (1 != sscanf(argv[argc-2], "%lu", &width) ) ||
    ...
    {
      error(1, 0, "YUVToBMPStreamFilterByFormula <width> <height> [--fd <Unix file descr>] \n: (stdin, stream of YUV I420p) --> (stdout, stream of classical bmp)");
    }
  ...
  while( (firstc = fgetc(inputFP)) != EOF )
    { 
      ...
      OneYuvStream2OneBmpByFormula(width, height, inputFP, pBMdata, pBM);

      size_t ret = fwrite(pBMP, BMPsize, 1, stdout);
      if( ret != 1 ) {
	error(1, errno, "YUVToBMPStreamFilterByFormula write bmp to stdout failed??");
      }
      imgCount++;
      if(verbose) {
  ...
  fprintf(stderr, "YUVToBMPStreamFilterByFormula transformed %u images.\n", imgCount);
  fclose(inputFP);
  fclose(stdout);
