...
(/usr/bin/time --verbose ./yuvSelectMulti --seline-fmt '%*d %d %*d %*d %*d %*f' --bmp-out-fd 5  \
		  "${w}x${h}" 3 4 \
		  3<${fwantfile} 4<PIPE.yuv 5>/dev/null; \
...
/usr/bin/time --verbose ffmpeg -hide_banner -loglevel quiet -y -an -i $movf PIPE.yuv
echo 
echo ffmpeg exits
