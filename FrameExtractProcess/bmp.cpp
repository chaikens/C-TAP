...
{ //C++ vs Python reminder in C++, instead of this.classmemname
  ...
  */
void BMclass::looksee( const char *msg, const char* keepfile)
{
  const char *fn = "/tmp/BMclassShowOff.bmp";
  if ( keepfile ) { fn = keepfile; }
  string fns(fn);
  write(fn);
  if (!msg) {
    cerr << "BMclass::looksee() is trying show you a bmp with ffplay." << endl;
  }
  else {
    cerr << msg << endl;
  }
  cerr << "Exit that window to continue." << endl;
  string playcmd("ffplay -loglevel quiet ");
  string semi(" ; ");
  string rmcmd("rm ");
  string cmd = playcmd + fns;
  if( !keepfile )
    cmd = cmd + semi + rmcmd + fns;
